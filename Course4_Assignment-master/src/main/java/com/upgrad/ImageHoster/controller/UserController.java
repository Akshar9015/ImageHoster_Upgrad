...
public class UserController {
    ...
    public String signUpUser(@RequestParam("username") String username,
        ...
        if(userService.getByName(username) != null) {
            errmsg.put("username", "username has been previously registered.");
            model.addAttribute("errors", errmsg);
            return "users/signup";
        }

        if(username.length() < 6 && password.length() < 6) {
            errmsg.put("username", "need to be 6 characters or longer");
            errmsg.put("password", "need to be 6 characters or longer");
            model.addAttribute("errors", errmsg);
            return "users/signup";
        }
